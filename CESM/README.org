...
We replace the =shr_orb_mod.F90= file from [[https://github.com/ESCOMP/CDEPS/blob/main/share/shr_orb_mod.F90][ESCOMP/CDEPS/shr_orb_mod.F90]] and add necessary data files to the same folder(s). Note that the =shr_orb_mod.F90= files differ slightly between ESCOMP/CDEPS and [[https://github.com/ESCOMP/CESM_share/blob/main/src/shr_orb_mod.F90][ESCOMP/CESM_share/src/shr_orb_mod.F90]]. Assuming that the lastest version is desired, we have merged the latest changes from both repositories.
...
2. Verify if the files are identical and create a backup copy of each unique
   =shr_orb_mod.F90= file to fall back on---in case of issues with our updated
   file.
   #+begin_src sh
     diff -u /path/to/my_cesm_sandbox/share/src/shr_orb_mod.F90 /path/to/my_cesm_sandbox/cdeps/share/shr_orb_mod.F90 
     cd share/src
...
To do it manually (for more control and to re-use the data files):
   ...
   3. Update our =shr_orb_mod.F90=:
      1. Search for =TODO= within the file.
      2. Update the filepath in the line that starts with =call readdata= to
      ...
      3. If you decided to use binary files (step 2 above),
         update the =call readdata= line to =call readbindata= and update the filename appropriately.
   4. Copy our Fortran file to each directory that contains =shr_orb_mod.F90=.
      For example,
      #+begin_src sh
        cd 'share/src'
        cp '/path/to/my_cesm_toolbox/orb/shr_orb_mod.F90' .
        cd '../../components/cdeps'
        cp '/path/to/my_cesm_toolbox/orb/shr_orb_mod.F90' .
        # etc.
      #+end_src
